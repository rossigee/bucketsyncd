...
### Fixed
- `NewWebDAVClient` now converts `webdav`/`webdavs` URL schemes to `http`/`https` for the underlying `gowebdav` client, which does not support the `webdav`/`webdavs` schemes directly
